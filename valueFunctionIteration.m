...
for n = Settings.nCheck:-1:1
    ...
    while (vSdiff>Settings.tolInner && iter < Settings.maxIter)
        ...
        iter = iter+1;
        logvS = log(vS(n,:)');
        pStay(n,:) = normcdf(logvS, -.5*Param.omega^2, Param.omega);
        partialExp = 1-normcdf(.5*Param.omega^2-logvS, 0, Param.omega);
    ...
    pEntry(n,:) = normcdf(logvS - log((1+Param.phi(n))), -.5*(Param.omega^2), Param.omega);
